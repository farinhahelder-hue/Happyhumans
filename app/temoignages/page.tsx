      ...
      <main>
        ...
        <section className="relative flex h-[55vh] items-end overflow-hidden bg-stone-900 md:h-[65vh]">
          ...
          <div className="relative mx-auto w-full max-w-5xl px-6 pb-12 md:px-10">
            <p className="mb-3 text-xs font-bold uppercase tracking-[0.22em] text-amber-400">{c.get('grid_badge')}</p>
            <h1 className="text-4xl font-serif font-light leading-[1.1] text-white md:text-6xl">{c.get('page_title')}</h1>
            <p className="mt-4 max-w-xl text-base text-stone-300">{c.get('intro_text', undefined, { multiline: true })}</p>
          </div>
        </section>
        ...
        <section className="bg-[#f7f4ef] py-20 md:py-28">
          <div className="mx-auto max-w-6xl px-6 md:px-10">
            {loading ? (
              ...
              <p className="text-center text-stone-500 py-16">{c.get('grid_empty_state')}</p>
            ) : (
          ...
          <div className="mx-auto max-w-5xl">
            <div className="mb-10 text-center">
              <div className="inline-flex items-center gap-2 bg-[#0077b5] text-white text-xs font-bold px-3 py-1.5 rounded-full mb-4">
                ...
                {c.get('linkedin_badge')}
              </div>
              <h2 className="text-2xl font-serif font-light text-stone-900 md:text-3xl">{c.get('linkedin_title')}</h2>
              <p className="mt-3 text-sm text-stone-500">{c.get('linkedin_subtitle', undefined, { multiline: true })}</p>
            </div>
            ...
            {c.linkedin_footnote && <p className="text-center text-xs text-stone-400 mt-6">{c.get('linkedin_footnote')}</p>}
            {c.linkedin_intro_note && <p className="text-center text-xs text-stone-400 mt-4">{c.get('linkedin_intro_note')}</p>}
          </div>
        </section>
        ...
        <section className="bg-[#2f6b61] py-16 text-center">
          <div className="mx-auto max-w-xl px-6">
            ...
            <p className="mb-7 text-sm text-emerald-100">{c.get('cta_subtitle', undefined, { multiline: true })}</p>
